...
| **PAY**   | _(none)_   | Payment integration — Stripe/Square, processor-as-clearing-account, hosted checkout                                       | **Dev-ready — see execution** |
...
unique insert (cheap, pre-journal); the same object arriving from *both* a webhook and the poll is
...
- **The webhook endpoint does not use the global `Idempotency-Key` middleware.** Stripe/Square won't
  ...
  `withGlobalIdempotency`) — deliberate, because here the processor id *is* the idempotency key
  ([D-85](#d-85)/F9).
