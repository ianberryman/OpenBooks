...
| Gate   | `yarn check` passes — 2,250 tests across 191 files, ~2.5 min; three milestone E2Es (`cash-basis.spec.ts`, `quickbooks-import.spec.ts`, plus the M1–M4 narratives) pass against a real stack                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Push   | **87 commits ahead of `origin/develop`, unpushed** — needs credentials this machine does not hold                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Next   | **Phases 1–3 are DONE — launch bar met.** Phase 1 (INV delivery), Phase 2 (K, cash-basis) and Phase 3 (QuickBooks CSV import: chart + contacts + opening trial balance → one opening journal, fiscal year auto-generated) are all committed with green gates and browser narratives. Phase 2's E2E caught a real transport defect (`basis` never crossed the wire schema; fixed `2cd50df`). QB import is CSV-only by decision — `.qbo`/OFX deferred. **Next: Phase 4 — automate the busywork: INV recurring + dunning first, then OCR (O, decided to use a pluggable OcrProvider).** Correction to an earlier note: Phase 1 did **not** ship a scheduler — it shipped StorageProvider, the email send-path, `org_branding`, `invoice_deliveries` and the hosted page. The scheduler is OB-127 and is the **first** Phase-4 deliverable. **The schema (OB-122) is now BUILT** — `0008_recurring_dunning` (five tables, codegen + registries + tripwires, gate green, `258e382`). DAG remaining: the daily in-process tick (OB-127) → recurring materialisation (OB-128, reuses `approveArDocument`) + dunning (OB-129, reuses the `sendInvoice` path, once-per-stage via `dunning_sends`) → routes + UI. Sequenced ahead of Stripe by decision. See [Release plan](#release-plan--post-m4-sequencing). |
