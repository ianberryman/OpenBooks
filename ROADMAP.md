...
| ---- | --------- | ------------------- | ----------- | -------- |
| **T1 — table stakes** | Live bank feeds + credit-card/liability reconciliation | file-import only (`feed_source ENUM('file')`), asset-only recon | QBO/Xero/QBD all live | **OB-227 (below)** — `BankFeedProvider` seam, **Stripe Financial Connections**, **v1 = bring-your-own account only** (managed deferred); recon generalisation from [any-account-type](#follow-up--import--reconcile-should-work-for-any-account-type-future) |
...
### Follow-up — live bank feeds via a `BankFeedProvider` seam; Stripe Financial Connections first (OB-227, future)
...
### Follow-up — statement-of-account & report export table-stakes (OB-220, future)

Two small, independent, high-perceived-completeness gaps that all three competitors ship and
OpenBooks lacks entirely. **(1) Customer statement of account** — a per-customer list of
invoices/payments/running balance over a period, delivered via the existing `delivery`/`branding`
PDF + email path (INV); the data already exists (invoices + `ar_allocations` + the AR aging
service), so this is assembly, not new subledger. **(2) Report CSV/Excel export** — every report
service (`modules/reports/*`) already computes structured rows; add an export endpoint + a
list/report "Export" action (currently a deliberate omission across the redesigned lists). Smallest
of the set, broad reach. Both reuse existing infrastructure and add no new posting path.
...
- **The incumbents don't treat it as day-one either.** QBO gates it at **Essentials** (not Simple
  ...
  three put it *behind* their entry tiers — it is a mid-tier, segment-specific feature (importers/
...
- **It is inconsistent with the current US-domestic direction.** Every other integration choice is
  ...
  non-US market (in which case it must come *before* the US integrations, a different product bet).
...
well-bounded, segment-motivated problem, it is a strong open-source contribution candidate — *if* the
...
cost-*layer* state that changes as sales draw layers down, and OpenBooks **cannot mutate** posted
records. Weighted-average maps cleanly onto an **append-only movement ledger**: each movement appends
a signed `(qtyDelta, valueDelta)`, on-hand is a running sum, and the unit cost is *derived*
...
recorded; traditional perpetual systems *mutate history* to recompute. OpenBooks resolves it with the
...
*backorder* is properly a **fulfillment-timing** concept (order now, ship — and only then move
...
moving average, or the item `default_cost` when it has *never* held stock — driving on-hand qty
...
over movements (any snapshot table is a rebuildable *cache*, like the journal-sequence counter is its
