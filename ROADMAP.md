...
| **SELFHOST** | _(none)_   | Production self-host readiness — SMTP email, clean upgrade path, provider-neutral durable queue, S3-compatible storage, k8s/Helm, backups, operator docs, CI (OB-229…231, 244…250)        | **Scoped — not started**       |
...
|        |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Branch | `develop` at `036084d` (OB-226 custom role builder), pushed to `origin/develop`. The working tree is **not** clean: it carries a separate uncommitted ten99/StorageProvider/infra thread (Dockerfile, docker-compose, nginx, ten99/run-detail, contacts) that predates this session and is deliberately not part of the OB-226 commit.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
built in-house (OAuth 2.1, code+PKCE) with a dedicated external security review of OB-098 **not
budgeted** — an internal static analysis stands in (2026-08-02, findings + severity table under
the [OB-098 item](#wave-1--identity-and-the-event-backbone-3-parallel); no exploitable-today defect, five
hardening items, #1 consent-CSRF defense-in-depth + #4 schema bounds recommended first); **per-org**
event ordering (a counter taken
...
The **payment-processing / integrations screen** (the `/v1/processing` management surface from PAY —
connect a Stripe/Square processor, view `processor_connections`, the reconciliation/poll status, the
public pay-link + Pay button) predates the document-UI redesign thread and has **not** had the
shared card / breadcrumb / summary-endpoint pass the bills, invoices, estimates, and purchase-orders
screens received. It should get the same treatment for consistency: lifecycle/summary cards, the
routed detail layout, and the responsive card alternates (`useIsCompact`, not `md:hidden`) initiative
R established. **UI-only — the PAY backend is complete** (`modules/payments-processing/*`, the webhook/poll,
refunds, clearing+fee posting all built and gate-green); this is presentation only, no service or
schema change. Unscheduled; captured so the processing surface is brought up to the rest of the app's
bar rather than left as the one un-redesigned screen.

### Follow-up (NEXT) — mobile view review: make page headers sticky like the redesigned bill page (UI-only)
...
##### Self-host docs — structure for minimum support load (design direction)

The self-host docs are the support team, and the problem they must solve is a **config matrix** (many
provider choices × deployment methods) sitting on top of **many not-yet-built adapters** (`sqs`,
`anthropic`, `ses-inbound`, `smtp`). The failure mode that generates tickets is docs that are _either_
one linear path (doesn't fit the reader's choices) _or_ every permutation written out (a maze). The
structure below — assumed on **Fumadocs** (MDX, `<Tabs>`, callouts, `generateFiles`, `fumadocs-openapi`,
static Orama search, versioning) — avoids both. Ranked by ticket-reduction impact:

1. **Persona-branched golden paths (biggest lever).** One linear, copy-paste-correct happy path _per
   deployment persona_ — Evaluate-locally / Self-host-your-infra / Self-host-on-AWS / (Cloud) — chosen
   from a single decision landing page (`<Cards>`). Most tickets are a reader on the wrong path or a
   path with branches; a golden path per persona kills both.
2. **Tabbed variants for per-choice config — the "multiple options" mechanism.** Where a concept has N
   adapters, use `<Tabs>` on _one_ page (Email → `Log · SES · SMTP(coming)`; Storage → `Local · S3`;
   Secrets → `Local · AWS Secrets Manager`), with **persistent tabs (`groupId`)** so choosing "AWS"
   once keeps AWS selected across every page. One concept, N implementations, no page duplication.
3. **An adapter availability matrix as a first-class page.** A single _adapter · status · needs_ table
   so no one files a ticket trying to use an adapter that throws at construction. Given how many stubs
   exist, this is likely the #2 ticket source after "how do I start," and it is one table.
4. **Generate the leaves; never hand-author drift-prone reference.** Env/config reference from the
   richly-commented `.env.example` + the Zod unions in `config/config.ts` (via `generateFiles`); the
   **API reference from the already-drift-gated `openapi.json`** (via `fumadocs-openapi`, with try-it);
   the availability matrix from the provider `switch`es. Same discipline the drift gate enforces on the
   spec — the docs cannot silently diverge from the product.
5. **Inline caveats at the point of failure + an error-keyed troubleshooting page.** Every gotcha is a
   Fumadocs callout where the reader hits it (SES sandbox; `require_secure_transport=ON` rejects
   connections because TLS-to-MySQL is unimplemented; migrations are a discrete `docker compose run --rm
migrate`, never a boot step; the stub adapters throw at startup). Plus a Troubleshooting page keyed to
   **actual error strings** so a paste-and-Ctrl-F finds the fix — the highest-ROI page in the set.
6. **Versioned docs + static search.** Pin docs to the release (pre-release and moving, so
   "docs say X, my build does Y" is a self-inflicted ticket source); Orama static search ships with
   Fumadocs and needs no hosted account.

**The lever docs cannot pull — record it here so it is scoped with the docs, not after.** The single
biggest deflector is the _app_ telling the operator what is wrong: a **`config check` / preflight
command** that validates env, checks DB connectivity + the two-user split, and prints actionable errors
(`EMAIL_PROVIDER=ses requires AWS_REGION`) — leaning on the fail-fast `loadConfig` that already exists —
plus a "Verify your install" doc page. A good startup error deflects ten doc lookups; this is app work,
but it belongs in the same initiative.

**Proposed IA (sidebar):** Get started (Choose how to run · the per-persona guides) · Configuration
(Environment reference _generated_ · Providers & availability matrix · Email/Storage/Secrets/Queue/Bank
feeds/OCR, each `<Tabs>`) · Operations (Database & the two-user model · Migrations · Upgrading · Backups)
· Reference (API _OpenAPI-generated_ · Troubleshooting · Config validation) · Concepts (append-only
ledger · multi-tenancy · the provider pattern). Build order: 1 → 3 → 4 → 5, with the `config check`
command alongside.

##### Draft marketing copy + messaging direction (starter, not final)

A first pass at the site's copy, to hand a designer/copywriter rather than start from blank. **Voice:
plain and human**, short sentences, benefit-first — explicitly _not_ the internal-docs register (no
"immutable ledger", "append-only", "MCP", "inference calls", "reversing entries"). **Positioning
decisions taken here:**

- **Lead with practical features, not integrity.** The hero and top section sell the tangible daily
  jobs — get paid, pay bills, bank reconciliation, reports, the full sales/purchase cycle. The
  "can't be fudged / penny-accurate / own-your-data" trust material is a **reinforcement block lower
  down**, not the headline (an earlier draft led with ledger immutability; deliberately demoted — it
  reads as a feature to owners only _after_ they see what the app does).
- **Two audiences, one page:** owners ("do the boring stuff faster, stop worrying you got it wrong")
  and accountants ("all the depth you actually need"), each its own section.
- **AI sits near the bottom** as a nice-to-have, framed safety-first ("bring your own AI, you still hit
  post, we never hold your keys") — not a pillar.
- **No unverifiable claims** — no security certs, uptime, customer counts, or pricing until there's
  real data + legal sign-off.

**Hero:** _Invoices, bills, banking, and reports — all in one place._ — "OpenBooks handles the everyday
accounting for your business: get paid, pay your bills, reconcile the bank, and see how you're doing.
Simple enough for owners, deep enough for accountants." CTAs: Start free · See a demo · Read the docs.

**Top section — "What you can do":** Get paid faster (branded invoices, online pay via Stripe/Square,
auto-reminders) · Pay bills without the shuffle (photo → bill capture → approve/pay queue) · Let your
bank do the data entry (feeds + auto-match reconciliation) · Know where you stand (P&L, balance sheet,
cash flow, aging — cash _or_ accrual; budgets, 1099s, close) · Estimates, POs, recurring billing (the
whole cycle).

**For business owners:** everyday tasks in minutes; not-an-accountant help (manual entries pre-fill the
right accounts + show the effect, so "debit or credit?" never trips them up — ties to OB-239/239b);
everything reconciles itself. Tagline: _You run the business. We'll keep the books._

**For accountants:** cash + accrual side by side; month-end close (checklists, sign-off, locked
periods); adjustments, audit trail, 1099s, budgets vs. actual, fixed assets; accountant role + clean
client access. Tagline: _Bring your clients onto books built for real accounting._

**Trust reinforcement (moved down):** nothing changes behind your back (corrections, not erasers);
always right to the penny; your data stays yours (open source, open formats, export, self-host _or_
hosted — same app).

**AI (near bottom):** _Bring your own AI helper. You still hit "post."_ — reads + drafts, never posts on
its own, we never touch your AI keys.

**Honest comparison table** (OpenBooks vs. most cloud apps): posted entries can't be secretly changed ·
run it on your own machine · take your data and leave anytime · AI gets only what you allow · cash +
accrual/close/audit trail included, not upsold.

**Closing:** _The whole back office, handled._ Start free, or run it yourself today.

The full formatted draft (all sections, exact wording) lives in session history; this is the
distilled direction so the positioning survives into the build. Refine the hero headline with
alternatives, and pick a voice register (warm / punchy / dry-witty) before commissioning final copy.
...
contract + the three adapters + the service routing/skip/finalize + poll wiring; three parallel
Sonnet streams authored the disjoint web UI, the server property suite, and the E2E narrative).
What's in: **[D-237-8]** `fetchPayoutBreakdown` now returns a discriminated `PayoutBreakdownResult`
(`ready` | `awaiting_report`) and routes on the payout's own `automatic` flag inside the Stripe
adapter — no guessing; **[D-237-9]** the Stripe classifier fires only on `payout.paid`, so a
pending `payout.created`/`.updated` is ignored (no NormalizedProcessorEvent status field needed —
the simpler of the two options the scope offered); **[D-237-10]** a manual payout stages an
`awaiting_report` `payout_syncs` row carrying a `report_run_id`, and the daily poll's new
`finalizePayoutReports` sweep polls the async Stripe Reporting-API run (`POST /reporting/report_runs`
→ poll → download the File CSV → aggregate) and finalizes it to `posted`/`pending_review`/`skipped`
— the report fetched outside the row lock, then a `FOR UPDATE` re-check per row (the `postPayoutSync`
locking discipline); **[D-237-11]** a breakdown fetch that throws now records a **visible** `skipped`
row with `skip_reason` = the processor's own message (bounded to the column width) and the event
marks `processed`, never a swallowed `processor_events:'failed'` with no UI trace — the silent-failure
fix, shippable on its own. Schema: an **in-place edit of `0024_payout_sync`** (D-15) adds
`report_run_id VARCHAR NULL` and widens the status CHECK with `awaiting_report`; `generated.ts`
regenerated against a throwaway MySQL; no new migration file (so the `harness.test.ts` ordered list is
untouched), no new table/route/permission (so the grant/tenant/permission-matrix/cross-org tripwires
are untouched); `openapi.json` + web `schema.d.ts` regenerated, `yarn drift` clean. The gate exercises
all three routes deterministically through `fake.ts` (D-102): a payout id containing `manual` →
`awaiting_report`, `boom` → a failed fetch (skip-and-flag), a report id containing `pending` → an
unfinalized report; real Stripe's Reporting-API + File-download code is believed-correct-from-the-docs,
sandbox-only. **Deliberate lean edges, flagged:** a breakdown-failure `skipped` row is terminal (the
`selectPayoutSyncByExternal` guard short-circuits a redelivery), so a _transient_ fault is made visible
but not auto-retried — an operator re-triggers, consistent with the existing non-usd `skipped`
semantics; a `failed` Stripe report run is treated as `pending` (the row waits, a persistently-stuck
run is operator-visible) rather than surfaced with its own reason; and the report CSV column mapping
(`reporting_category`/`net`/`fee`) follows Stripe's documented summary schema, unproven against a live
run. **DB-reset caveat unchanged:** the in-place `0024` edit means a prod deploy is again a full DB
reset (down -v + fresh migrate), not a plain rebuild — not yet done (build only; no deploy this session).

---

#### Original scope (as written before the build)

The original scope follows, retained for the decision record.

##### manual payouts + the async Reporting-API breakdown (OB-237b) — the plan
...
### Follow-up — pre-classify `cash_basis_role` in the starter chart (OB-240)

Set `cash_basis_role` on the unambiguous accounts of the **opt-in starter chart** (OB-039,
[chart-templates.ts](packages/server/src/modules/accounts/chart-templates.ts), [D-23](#d-23)) so an
org that adopts it gets **correct cash-basis reporting out of the box** instead of 65 rows sitting
`null` until the setup nudge is run on each.

**Why the "created `null`" rule doesn't bind here.** `cashBasisRole` is deliberately null at account
_creation_ because at that point "the user may not know how the account will be used"
([accounts.ts:229](packages/shared-types/src/accounts/accounts.ts:229)) and the distinction cuts
across `type` (checking and prepaid-expenses are both `asset`; only one is cash). That rationale
**evaporates for a curated template** — the starter chart's accounts are known, standard ones whose
role is unambiguous. This is the **same shape of decision** the file already makes for the four contra
accounts whose `normalBalance` doesn't follow from `type` ("a starter chart that got them wrong would
be worse than no starter chart… reports with the wrong sign and reads as a data problem"). Cash-basis
role reports **empty/wrong** if omitted, in the one place the curator knows the answer.

**It serves a launch feature.** Cash-basis P&L (K/OB-154) and the cash-flow statement both key off
this flag — `cash-flow.service.ts` unions "every account flagged `cash_basis_role = 'cash'`," "the same
hint OB-154's transform reads." Most customers file cash-basis (see the release plan), so a pre-filled
template is the difference between cash-basis working by default vs. after a chore users won't know to
do. Stays consistent with the invariants: the field is nullable + user-overridable and the template is
opt-in/unenforced (D-23), so a pre-fill is a **default, not a lock** — the setup nudge still catches
user-added accounts and any left null.

**What to tag (small set — most of the 65 stay `null`):** cash & equivalents → **`cash`** (`1010`
checking, `1020` savings, `1030` undeposited funds, plus any petty-cash/clearing); the pure-accrual
**holding** accounts → **`accrual`** (`1300` prepaid expenses, `2100` accrued liabilities, the
deferred/unearned-revenue account, customer deposits). **Confirm AR/AP against OB-154's transform**
before tagging — accrual holding accounts conceptually, but whether the transform wants them flagged or
handles them structurally is a detail to verify, not assume. Ordinary revenue/COGS/expense stay `null`
(recognised via the cash account they pair against, not by a role of their own).

**The one seam that isn't free.** `createAccountRequestSchema` deliberately **omits** `cashBasisRole`
(it lives on the _update_ schema only) and the repository create hard-codes `cash_basis_role: null`
([accounts.repository.ts:147](packages/server/src/modules/accounts/accounts.repository.ts:147)). So
`applyChartTemplate` can't pass it through the public create — either set it via the **patch path**
after creating each account, or extend the **internal** template-create path. No schema/migration
change (the column and enum exist since `0002_ledger`); the account-`type` `ENUM` is untouched, so **no
`generated.ts` regen**. Update the starter-chart application test and any chart-template snapshot to
carry the new roles; no new permission key, no route change, no catalog change.

### Follow-up — expand the MCP surface: all reports + presale reads, and draft-upsert for every AR/AP type (OB-241 / OB-241b)

The MCP suite today is 8 tools — 5 domain `*.list` reads, one report (`reports.trial_balance`), and two
propose-only writes (`journal.propose`, `work_queue.submit_proposal`), both landing a draft a human
posts. This widens it to the rest of the read surface and adds a document draft-write path — taking it
to **~27 tools**. Two parts with **different postures**, hence the a/b split; **OB-241 (reads) is
low-risk and ships first**, OB-241b is the posture-shift.

**OB-241 — all report + presale-document reads (pure reads, existing `.read` keys).** Add one MCP tool
per report operation already on `/v1/reports` beyond trial balance — `reports.profit_and_loss`,
`reports.balance_sheet`, `reports.aging` (direction param), `reports.general_ledger`,
`reports.cash_flow` (statement of cash flows), `reports.cash_flow_projection`,
`reports.budget_vs_actual` (all `reports.read`), and `reports.audit` (`audit.read`) — plus the
**presale documents**, the two non-posting pre-transaction types, as `list` + `get`:
`estimates.list`/`estimates.get` (`estimates.read`) and `purchase_orders.list`/`purchase_orders.get`
(`purchase_orders.read`). Each tool **calls the same service the HTTP route does**, inheriting its
tenancy scope and permission check verbatim. **Explicitly out:** the report **export**
(`GET /v1/reports/export`) is a binary file download, not an MCP shape; customer-statement **render** is
a write (stores an artifact) — its list is a possible later read, deferred.

**OB-241b — draft-upsert for every AR/AP type (the posture shift).** One `*.upsert_draft` tool per
document type — AR: `invoices`, `credit_notes`, `estimates`; AP: `bills`, `vendor_credits`,
`purchase_orders`, `expenses` (7 total) — each gated by that domain's existing **`.write`** key
(`bills.write`, `estimates.write`, `purchase_orders.write`, `vendor_credits.write`, `expenses.write`,
and invoices/credit-notes via `invoices/ar-documents.service`). This is the **document analogue of
`journal.propose`**, and it is safe for the same reason (D-100): a **draft document is inert** — no
ledger entry, nothing sent to a customer/vendor — and the **issue / send / approve / post** step stays
a **human action under a separate permission the agent does not hold** (the SoD already exists). So
unlike a journal, a draft document needs no `agents.review` detour: the **draft _is_ its own reviewable
artifact**, and a person commits it. The tools **reuse the existing `create*`/`update*` services**,
inheriting their balance/tenancy checks and — critically — their **draft-only update guard** (e.g.
`estimates.service.ts:285` "only that the estimate is still a draft"): confirm every type enforces it
and add it where missing, because that guard is the whole safety story.

**Forks to settle first (the decisions):**

- **Reports stay per-operation — SETTLED, not a single parameterized `reports.run(report, …)`.** The
  eight report tools only _look_ homogeneous: their params differ (`aging` a direction, `general_ledger`
  an account + range + paging, `budget_vs_actual` a budget/period, P&L/BS a basis and range-vs-as-of,
  `audit` its own window) and their **return shapes are unrelated objects**, so one tool would need a
  discriminated-params schema and return an `any`-shaped blob — harder for a model to call correctly, not
  easier. Two OpenBooks specifics seal it: the permission isn't uniform (**`audit` is `audit.read`**, the
  other seven `reports.read`), which breaks the suite's **one-tool → one `requirePermission` → one
  service** invariant that `permission-matrix.test.ts` asserts per tool; and `reports.trial_balance` is
  already a single-report tool, so consolidating would strand it as an exception. Named tools are also
  easier for the model to _select_ at n=8 — the tool name is the intent. (Reconsider only at a large,
  homogeneous family — 30+ reports sharing params/return/permission — where MCP _resources_ or one
  parameterized tool would beat flooding the list. The read-side mirror of the per-type draft-upsert
  decision below.)
- **Upsert identity + the draft-only guard (headline safety).** `upsert_draft` = **create when no id,
  update when an id is given**, and **hard-refuse any target not in `draft`** (issued/approved/sent/
  posted carry ledger or external effects). Recommend enforcing this in the shared service path both the
  route and the tool call, so the tool cannot be a second, weaker write path (the `no-journal-writes`
  discipline, one layer up).
- **Per-type tools vs. one generic `ar_document.upsert_draft`/`ap_document.upsert_draft`** — recommend
  **per-type** (each doc's line/field schema differs enough that one union schema would be worse to
  drive), reusing the shared `ar-documents.service` where it already unifies them.
- **Bound the large reads.** `general_ledger`, `trial_balance`, and `audit` can be huge — the tools must
  reuse the routes' **keyset paging / required date range**, never an unbounded dump. Flag GL especially.
- **No new catalog keys.** Every read reuses a `.read` key and every draft-upsert a `.write` key that is
  already seeded — the count stays put, and the agent receives them through OAuth scope∩role exactly as
  a human role would. Issue/send/approve/post remain the separately-held permissions.

**Sequencing:** OB-241 (reads) is purely additive read tools over existing services — ship it first.
OB-241b (draft-upsert) is the posture change (agents gain a direct draft-write path) — worth its own
review, and it depends on auditing each type's draft-only guard. Independent of OB-239's own journal
templates, though they rhyme (both are "compose a draft, a human commits").

##### Tripwire / registry checklist (MCP-only — no HTTP/schema surface)

`modules/mcp/tools.ts` (+~19 tools) + `host.ts` registration · the **MCP tool-registry / host tool-list
test** (count + names) · `test/enforcement/permission-matrix.test.ts` **MCP coverage** (each new tool's
`requirePermission` under an **existing** key — no catalog change, so `catalog.test`/`0001` untouched) ·
the `platform-oauth-mcp` E2E extended to exercise a new read and a draft-upsert. **What this does _not_
touch, unlike a route change:** no migration / `generated.ts`, **no new HTTP routes**, **no
`openapi.json` / web `schema.d.ts` / `yarn drift`** (MCP tools are not in the OpenAPI surface), no new
permission key. Draft-upsert tools return the created/updated draft's id + shape; all new tools ride the
D-118 typed-error-on-HTTP-200 host contract.
...
### Follow-up — replace the `anthropic` OCR extraction adapter with AWS Textract (unscheduled)

The bill-capture pipeline's hosted extraction adapter should be **AWS Textract**, not Anthropic. Today
the `DocumentExtractionProvider` seam (`providers/extraction/`) ships `deterministic` (a real
key:value parser that drives the gate) and `anthropic` — the latter a **throwing stub**, the placeholder
for the hosted OCR path. Replace that stub with a real **Textract** adapter (`providers/extraction/textract.ts`,
called over global `fetch` per the narrowest-surface rule — no vendor SDK, matching the other provider
adapters). Two reasons it is the right target: (1) Textract is a purpose-built document-OCR/analysis
service, a better fit than a general LLM for structured field extraction; and (2) it keeps OpenBooks
free of **model credentials** — the same posture [D-100](#d-100) took for automations (OpenBooks holds
no model keys and makes no inference call), which an `anthropic` extraction adapter would violate. It
also sits alongside the other deferred **AWS-hosted** adapters (`aws-secrets-manager`, `ses-inbound`,
`s3` storage), so a single hosted-on-AWS deployment story covers them together. UI/flow unchanged — this
is one new adapter behind the existing seam plus its config wiring; the staging → review → draft-bill
path is untouched. Supersedes the `anthropic` OCR line in the [OSS good-first-issue](#open-source--self-host-readiness-ob-229ob-231) list.
...
**Direction change (owner, 2026-08-02): bank feeds should be a GLOBAL provider, not per-org.** This
reverses the BYO-only headline above and promotes the deferred **managed ("use ours") model** to the
plan. In provider-pattern terms it moves `BankFeedProvider` from a **per-org factory**
(`bankFeedProviderFor(source, deps)`, resolved against a per-org restricted key in the secrets store,
exactly the payment/1099 shape) to a **process-wide singleton** (config-selected once at boot, the
`storageProvider()`/`secretsProvider()` shape) — OpenBooks holds **one platform Stripe Financial
Connections credential**, not an org's own. What this pulls in, and must be scoped _with_ it rather
than after: (1) the FC per-account charge ($0.30/account/month) now lands on **OpenBooks'** bill, so
**per-org usage metering + an OpenBooks-side billing/subscription surface become prerequisites** — the
exact cost-isolation BYO existed to avoid, so this trades operational simplicity for the metering/
billing system v1 deliberately skipped; (2) `bank_feed_connections` keeps per-org **linked accounts**
but the `credential_source` field flips its default `bring_your_own` → `platform` (the field already
exists, so no schema change — the reason it was added); (3) the pull job and the whole reconciliation
pipeline are **unchanged**. BYO can stay a supported `credential_source` for an org that prefers its
own Stripe relationship, but the **default and primary path becomes the global platform account**.
Net: adopt the managed model, and sequence the metering/billing prerequisite as part of it.

**Pricing decision (owner, 2026-08-02): the platform fee bundles 3 connected institutions;
cap-and-upgrade, not usage billing.** Every plan includes **3 institutions** under the managed feed;
connecting more is a **tier upgrade**, not a metered per-account charge. This deliberately reduces the
"metering + billing surface" prerequisite above to a **per-tier connection quota** — a count the
existing permission/limit machinery enforces — so the managed model ships **without** building a
usage-billing system. Cost is a rounding error at this bundle (3 institutions ≈ **$0.90/org/month**
COGS at FC's $0.30/institution/month, absorbed into the fee), and the hard cap protects margin from
the rare power user. **Unit caveat (must confirm before launch):** the bundle is denominated in
**connected institutions** to match Stripe FC's billing unit (per institution per account-holder ≈
per connection) and what a user intuitively calls "my bank"; verify against a **live FC invoice** that
Stripe meters per-institution and not per-account (the same open unit question this section already
flags), or the quota and the COGS will not line up.
...
### Follow-up — 1099 export is IRIS CSV only for now (OB-254, scoped)

**The decision.** The one 1099 filing/export path OpenBooks offers for now is an **IRIS CSV** — the IRS
Information Returns Intake System Taxpayer Portal template the org uploads itself (≤100 forms per
submission, free, needs only a one-time IR-TCC). Two other paths are deliberately **not** built: a live
transmit adapter (IRIS A2A / Tax1099 / Track1099) stays deferred per [D-228-7](#d-228-7), and there is
**no printable Copy A for mail filing** — sharpening D-228-7's "no red-ink Copy A" from a rendering note
into a product decision.

**Why CSV-only is right-sized.** 1099s are filed **once a year** (1099-NEC to both the recipient and the
IRS by **Jan 31**; 1099-MISC to the IRS by **Feb 28** paper / **Mar 31** e-file), so a manual
export→upload step is acceptable friction for an annual task and not worth a certified network
integration each org exercises once. Mail filing is a dead end regardless: the **e-file mandate** bars
paper for any filer at **10+ information returns** (aggregate, since TY2023), and Copy A cannot be
self-printed — it must be the official scannable red-ink form or a Pub. 1179-approved substitute. IRIS
CSV serves the small filer and the 10+ filer alike, for free.

**Current state (context).** The `manual` adapter today emits a placeholder pipe-delimited flat file
(`encodeManualTransmission`, self-described as "not a claim of IRS conformance"), and `efileTen99Run`
**builds that artifact and discards it** — it is neither IRS-format nor downloadable. Recipient **Copy
B** renders and now downloads correctly (the `/artifacts` nginx proxy, the Copy B link repointed to the
on-demand `GET /v1/ten99/forms/{id}/pdf` render route, and a writable persistent storage volume all
landed — **uncommitted** as of this entry), but it is a branded substitute statement missing required
content (see the Copy B follow-up).

**The work.**

1. **Payer EIN capture (prerequisite for _any_ filing path).** No payer TIN is stored anywhere today —
   `OrgIdentityRow` is `{ name, createdAt }` only, and the renderer input carries no payer TIN. Add an
   **encrypted** EIN column (PII-at-rest rule: a row-level TIN is an encrypted column via
   `crypto/field-encryption.ts`, **not** the secrets store), a settings field to set it, and thread it
   into the export and the Copy B render input. Schema change ⇒ orchestrator-owned (codegen against a
   throwaway DB; `MUTABLE_TABLES` + the schema-set tripwires).
2. **Real IRIS CSV builder.** Replace the `manual` adapter's placeholder output with the IRIS Taxpayer
   Portal CSV layout — **one file per form type** (1099-NEC and 1099-MISC have distinct templates);
   payer block from the EIN + branding; **full** (untruncated) recipient TIN via the decrypt-at-boundary
   path `efileTen99Run` already uses. Keep it a pure, deterministic, byte-stable function (the
   `manual.ts` property-test idiom).
3. **Persist + download.** Store the CSV artifact (storage writes work now) and surface a "Download IRIS
   CSV" action on the run detail, mirroring the Copy B route just fixed. The run status stays
   `ready_to_file` (`manual`'s terminal state).
4. **Gate.** Byte-stable CSV test; permission reuse (`ten99.write`); the schema-set tripwires for the new
   EIN column.

**Scope boundaries.** The `Form1099Provider` seam and `Form1099ProviderKind = 'manual' | 'iris'` are
unchanged — this is `manual` doing its job properly, not a new kind. **Out:** live transmission, ATS
certification, Copy A, state CF/SF codes, correction-file layout. The **Copy B substitute-statement
fixes** (payer TIN + the required "Instructions for Recipient" legend) are tracked as their own
follow-up and share only the payer-EIN prerequisite.

**Effort:** **small–medium** — the EIN migration + settings field is the one orchestrator-gated piece;
the CSV builder and the download are an add-only slice on the existing route/render patterns.
...
**BUILT, COMMITTED (`036084d`), PUSHED to `origin/develop`, and DEPLOYED to prod — gate-green
(server 2,372 + web/shared-types 672 tests; the +5 server ops and the 78th catalog key ripple through
every pinned tripwire, all updated).** Deployed to the prod Compose stack via the sanctioned D-15
reset (`down -v` + `up --build`, since the `roles.write` seed is an in-place `0001` edit) — migrate
re-applied `0001…0999` fresh and prod verified: catalog **78**, `roles.write` present and held by
`owner` only (`bookkeeper` correctly excluded), `GET /v1/permissions` + `GET /v1/roles` live and
gated (401) behind the web at `:8089`. (The redeploy also carried a **separate uncommitted
ten99/StorageProvider/infra thread** that was already on disk — not part of this commit.) An org admin
composes a
...
### Milestone SELF-HOST — production self-host readiness (SELFHOST)

**The gap between "boots in Compose" and "a business can run this in production, off-AWS."** The
runtime is already cloud-agnostic — a plain Node container + MySQL 8.4, with `local` storage/secrets +
`in-process` queue as AWS-free defaults (see [can-it-run-non-AWS](#operations--evaluate-aws-deployment-architecture-ob-235-future)
and OB-243's verified findings). This milestone collects what still stands between that and an honest,
supportable self-host, and is the prerequisite for any paid self-host support/business tier. It
**umbrellas** the existing OSS-readiness items (OB-229…231, detailed below) and adds the deployment
holes found in the 2026-08-02 portability review. None of these touch the schema-immutability model,
the tenant scoping, or the permission catalog — they are adapters, ops, and docs.

| Item                                         | Severity | Ticket           | Note                                                                                                                                                                                                                                   |
| -------------------------------------------- | -------- | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Outbound email (SMTP adapter)**            | Blocker  | OB-229           | Default `email=log` doesn't send → no invoice delivery, dunning, or statements. `smtp`/nodemailer unbuilt; `ses` is real but AWS-only.                                                                                                 |
| **Clean upgrade path (in-place migrations)** | Blocker  | **OB-250** (new) | Pre-release, migrations are edited in place (D-15) → an updated migration forces a **DB reset**, not a forward upgrade. No upgrade story until the D-15 inversion at first release. The least-visible, most-likely-to-bite hole.       |
| **Operator deploy guide (env, TLS, backup)** | Blocker  | **OB-244** (new) | Setup (`SESSION_SECRET`, `SECRETS_ENCRYPTION_KEY`, the two DB users) lives only in `CLAUDE.md`/Compose comments. DB users are `REQUIRE SSL`, so a non-TLS MySQL breaks bootstrap. No documented backup/restore.                        |
| **Provider-neutral durable queue**           | Scale    | **OB-245** (new) | `in-process` runs jobs in the enqueuing process and drops in-flight work on restart (known-gap-7); `sqs` throws. Blocks >1 app instance. A Redis/Postgres adapter — the non-AWS analog of OB-235's queue item, shares the OB-242 seam. |
| **Shared / S3-compatible object storage**    | Scale    | **OB-246** (new) | `local` isn't shared across nodes; `s3` is hardcoded to AWS (`new S3Client({region})`, no `endpoint`/`forcePathStyle`). Add those two config fields to unlock MinIO/R2/Backblaze/GCS.                                                  |
| **k8s / Helm deploy artifacts**              | Ops      | **OB-247** (new) | Only `docker-compose.yml` ships; `infra/terraform` is AWS-only and never applied. No Kubernetes/Helm path.                                                                                                                             |
| **Observability (metrics/dashboards)**       | Ops      | **OB-248** (new) | Structured logs + `/health` only — no metrics endpoint or reference dashboards for a self-host operator.                                                                                                                               |
| **Cross-tenant operator/admin tooling**      | Ops      | **OB-249** (new) | Nothing lets a self-host admin manage orgs/users instance-wide (create/suspend orgs, reset a locked-out owner).                                                                                                                        |
| **CI reconstruction**                        | Ops/OSS  | OB-230           | `.github/workflows/ci.yml.disabled` is a 0-byte file — external PRs are unvalidated and the pinned tripwires go unenforced on contributions.                                                                                           |
| **Roadmap → Issues/ADRs migration**          | OSS      | OB-231           | Move `OB-NNN` work to Issues/Projects + `D-NN` decisions to `docs/decisions/` ADRs; freeze this file.                                                                                                                                  |

**Sequencing:** the two true deployment blockers are **outbound email (OB-229)** and **the upgrade
path (OB-250)** — without the first a self-host can't do core accounting work, and without the second
an operator can't safely take an update. Everything below Scale is polish or horizontal-scale
(the latter shared with OB-235/OB-242). A paid self-host support tier (see the M7 pricing thread and
OB-243) should ride **behind** this milestone, not ahead of it — support for a rough install is a
treadmill; support for a polished one is margin.
...
headline claims in `README.md`. **(These three are items of the [SELFHOST milestone](#milestone-self-host--production-self-host-readiness-selfhost) above; the detail follows.)**
...
**Self-host email fallback chain + the SMTP/OAuth boundary (owner direction, 2026-08-02).** Hosted uses
OpenBooks' managed **Postmark** account with brokered per-org domain authentication ([OB-252](#hosted--per-org-sender-identity--self-service-domain-authentication-ob-252-future));
**self-host gets none of that automatically** and falls back, in recommended order: **`smtp`** (default —
host/port/user/pass; covers a generic server _and_ the org's own Outlook/Gmail via an **app password**) →
**BYO `postmark`** (the _same_ `fetch` adapter the hosted plane builds, pointed at the operator's own
Postmark account + their own domain auth — reuse, no new code) → **`log`** (the no-send floor for
eval/onboarding), with **`ses`** remaining for an AWS self-host. **Domain authentication (SPF/DKIM/DMARC)
is the operator's own responsibility in every self-host path** — contrast the hosted plane where OpenBooks
brokers it. **Precision to hold: `smtp` is basic-auth only** — username/password, _including vendor app
passwords_ — which is **not OAuth**. App-password SMTP works today but is being deprecated (Microsoft is
disabling M365 SMTP basic auth; Google gates app passwords behind 2FA and admin policy). **True
Gmail/Outlook OAuth is a separate deliverable**, and the SMTP adapter does not bring it along: its real
cost is the token machinery — OAuth app registration (Google Cloud / Microsoft Entra), the consent/connect
flow, per-org **refresh-token** storage in the secrets store, and access-token refresh — after which the
last mile is a free choice between **SMTP `XOAUTH2`** and the vendor **REST API** (Gmail API / Microsoft
Graph `sendMail`, `fetch`-based, no SDK). Scope OAuth as its own item when the basic-auth sunset bites; a
self-hoster who points `smtp` at their _already-authenticated_ Outlook/Gmail mailbox (app password)
sidesteps both OAuth and domain-auth setup in the meantime, because the vendor already signs their mail.
...
**Decision recorded (2026-08-02): run `SECRETS_PROVIDER=local` on AWS too — do _not_ select
`aws-secrets-manager`.** Two reasons, both current-state facts, not preference: (1) the
`aws-secrets-manager` adapter is an **unbuilt stub that throws at construction** (like `sqs`/`anthropic`),
so selecting it crashes the process at boot; and (2) **field-encryption for PII columns (the vendor TIN,
`crypto/field-encryption.ts`) is coupled to `SECRETS_ENCRYPTION_KEY`, which exists only under the `local`
provider** — under `aws-secrets-manager` it throws (D-228-2a, the deferred dedicated field key). So `local`
is load-bearing for **both** the credential store **and** every encrypted column. The sanctioned hosted
posture is therefore: keep `SECRETS_PROVIDER=local`, and hold the **`SECRETS_ENCRYPTION_KEY` itself in AWS
Secrets Manager / SSM**, injected into the task as the app key at boot — AWS-managed custody of the master
key, app-side AES-256-GCM into RDS, and encrypted PII columns keep working. Building the real
`aws-secrets-manager` adapter is only worth it _bundled with_ the D-228-2a field-encryption key, or
encrypted columns silently break the moment it is selected.

**Forward-compat for key rotation (do the cheap part now, 2026-08-02).** The at-rest envelope today is
exactly `iv(12) || authTag(16) || ciphertext` with **one static key and no version marker**, so rotating
`SECRETS_ENCRYPTION_KEY` means a decrypt-all/re-encrypt-all sweep over the `secrets` table _and_ every
encrypted column — there is no online path. Online rotation is **not** worth building for beta (pre-release,
tiny data, key already isolated in config), but two moves make it a cheap increment later instead of a
migration nightmare, and one is a regret-minimizer to do **before encrypted-data volume grows**:

1. **Add a 1-byte key-version prefix to the envelope** (`version || iv || authTag || ciphertext`). This
   turns future rotation into an incremental **dual-read** (decrypt with the version the row names,
   re-encrypt lazily or in a sweep) instead of a big-bang. Trivial now while there is almost no ciphertext;
   genuinely painful to retrofit later. This is the one thing to do now.
2. **Build the eventual `aws-secrets-manager` / D-228-2a work as _envelope encryption_ (a KMS-managed KEK
   that wraps a data key), not a second static key** — then rotation is re-wrapping the data key (cheap, no
   data touched) and KMS gives audited, scheduled KEK rotation for free.

Rotate **on event** (suspected compromise, personnel change), not on a calendar, until a compliance
framework demands a cadence. **Footgun to document:** "rotate the key in Secrets Manager/SSM" alone does
**not** rotate anything — with no versioning + no re-encrypt path it just makes all existing ciphertext
undecryptable. A data-encryption key is not an API credential; the managed-rotation feature does not apply
to it.
...
### Security — at-rest key rotation via envelope encryption (OB-251, future)

**Placeholder — a small forward-compat change now, the full capability later.** Today one static
`SECRETS_ENCRYPTION_KEY` (`sha256` → AES-256-GCM) encrypts **both** the `secrets` store and every
encrypted PII column (`crypto/field-encryption.ts`), and the envelope is exactly
`iv(12) || authTag(16) || ciphertext` — **no key-version marker**, so rotating the key means a
decrypt-all/re-encrypt-all sweep with no online path. Online rotation is not worth building for beta
(pre-release, tiny data, key already isolated in config), but the retrofit gets expensive as encrypted-
data volume grows, so this ticket carries the plan:

1. **Now (cheap, regret-minimizing):** add a **1-byte key-version prefix** to the envelope
   (`version || iv || authTag || ciphertext`), so a future rotation is an incremental **dual-read**
   (decrypt with the version the row names; re-encrypt lazily or in a sweep) instead of a big-bang.
   This is the one part worth doing before there is much ciphertext — it is trivial now and painful to
   retrofit later. Both write paths (`providers/secrets/local.ts`, `crypto/field-encryption.ts`) share
   the envelope, so it is one change in one place.
2. **Later (with D-228-2a / the hosted `aws-secrets-manager` adapter):** build it as **envelope
   encryption** — a KMS-managed **KEK** (key-encryption key) that wraps a **DEK** (data-encryption key),
   never a second static key. Then rotation is **re-wrapping the DEK** (cheap, no data touched) and KMS
   supplies audited, scheduled KEK rotation for free. Building the KEK/DEK layer in from the start of
   that work avoids a second re-encrypt migration; bolting it on afterward does not.

**Cadence:** rotate **on event** (suspected compromise, personnel change), not on a calendar, until a
compliance framework (SOC 2, enterprise procurement) demands one. **Footgun to document in the ops
runbook:** "rotate the key in Secrets Manager/SSM" alone rotates **nothing** — with no versioning and no
re-encrypt path it simply makes all existing ciphertext undecryptable; a data-encryption key is not an
API credential and the managed-rotation feature does not apply to it. Coupled to [OB-235](#operations--evaluate-aws-deployment-architecture-ob-235-future)
(the `SECRETS_PROVIDER=local` decision + its forward-compat note) and to **D-228-2a** (the deferred
dedicated field-encryption key), which is the natural moment to land the KEK/DEK design. A break-glass
decrypt-all/re-encrypt-all runbook is worth writing even before any automation.

### Operations — rearchitect event handling for multi-tenant cloud scale (OB-242, future)

**Placeholder — a rearchitecture design pass, not a build.** The event backbone (M5 `event_log`
outbox) and the automations detection loop (Q) were built for correctness on a single-tenant
in-process footprint, and both have scaling limits that the hosted plane — **hundreds to thousands
of orgs, each with their own event-triggered automations** — would hit hard. This todo is the
decision pass that must land **before** (or alongside) a managed offering scales past a handful of
tenants. It is coupled to [OB-235](#operations--evaluate-aws-deployment-architecture-ob-235-future):
several fixes below _are_ the durable-queue/worker-tier work that evaluation scopes.

The concrete limits, all grounded in the current code:

- **Detection is poll-only, on a single daily tick, with no emit→sweep coupling.** `emitEvent`
  (`modules/events/outbox.ts`) only inserts an `event_log` row; nothing enqueues the automations
  sweep on emit, and the sweep runs solely off `startDailyTick`'s `setInterval(ONE_DAY_MS)`
  (`modules/scheduling/tick.ts`) plus a boot fire and the manual `run-due-work`. **Worst-case
  latency from event to automation reaction is ~24 hours.** No near-real-time path exists.
- **The event sweep is O(orgs × events × automations), fully sequential.** `fireEventAutomationsForOrg`
  (`modules/automations/job.ts`) pages each org's `event_log` (capped `EVENTS_PER_SWEEP_LIMIT = 500`)
  and, for every event, loops _every_ active event-automation with an exact-string name match in nested
  `await`ed loops. At thousands of orgs a single tick's sweep will not complete meaningfully, and the
  500/tick cap bounds automation matching to ~500 events per org per day.
- **No horizontal scale: the in-process queue has no leader election.** The `sqs` adapter throws
  (`providers/index.ts`); under the default `in-process` adapter the **API** process runs the tick and
  handlers and the `worker` role is a de-facto no-op. There is no advisory lock at the tick level, so
  running more than one API/worker replica would **double-fire** every sweep. (The `work_items`
  `FOR UPDATE SKIP LOCKED` lease is the in-repo precedent to generalize.)
- **Delivery is lossy under failure and duplicative under crash.** The shared per-org `'automations'`
  change-feed cursor advances _outside_ the firing try/catch, so a handled failure **drops** the event
  (at-most-once, no retry), while a crash between a successful firing and the cursor advance **re-fires
  every listener** (duplicate `work_items`). There is no per-`(automation, event)` idempotency record,
  no retry/backoff, and no dead-letter path.
- **Matching is a full log scan; there is no topic/pattern index.** Every sweep reads the whole per-org
  log and filters in-handler by exact `eventName` equality — no `(org_id, name)` index driving it, no
  `invoice.*` prefix subscriptions.
- **`event_log` has no pruning.** `CHANGE_FEED_RETENTION_DAYS = 90` is documented but unenforced;
  the log grows unbounded, which becomes its own scaling wall.

Directions to weigh (options, not a prescription):

- **Emit→dispatch coupling** — on `emitEvent`, durably enqueue per-org (or per-event) work so
  automations react in near-real-time, demoting the daily tick to a backstop/redrive.
- **A real durable queue + worker tier** — make `sqs` (or equivalent) real so the `worker` role
  survives restart and scales out, with **leader election / claim-lease** so N replicas don't
  double-fire (generalizing the `work_items` lease). This _is_ OB-235's durable-queue item.
- **Sharded / parallel fan-out** — partition the sweep by org (hash across workers) so one slow or
  high-volume org can't starve the rest, and firings run concurrently rather than one sequential chain.
- **Index-driven matching** — index `event_log` by `(org_id, name)` and/or maintain a per-org
  subscription index so a sweep touches only subscribed event kinds; consider topic/prefix filters.
- **At-least-once with retry + DLQ** — per-`(automation, event)` idempotency plus bounded retry/backoff
  and a dead-letter path, replacing today's silent at-most-once drop.
- **Retention/pruning** — make the 90-day window real (archive + prune `event_log`).

Deliverable is a written recommendation (a decision `D-NN` + an ADR under `docs/decisions/`) with a
migration/rollout sketch — not code. It builds on the M5 event backbone (OB-100…) and the Q
automations detection loop (OB-200…210), and shares the durable-queue/worker seams with OB-235.

### Hosted — subscription-gated signup + tenant billing/entitlement (OB-243, future — the M7 pricing surface)

**The concrete build behind M7's open "hosted pricing/tiers" decision (`ROADMAP.md` §M7).** The hosted
model needs paid signup, and **none of it exists today** — this is greenfield. Verified against the
current tree: `POST /v1/auth/register` (`transport/routes/auth.ts:92`) is open, anonymous, self-service
(no `requirePermission`, only an `Idempotency-Key`) and atomically mints a user + org + Owner + session;
its body is a strict `{ email, password, displayName, org }` where `org` is `{ name,
fiscalYearStartMonth?, chartTemplateId? }` — **no plan/payment/billing field is even accepted**
(`shared-types/src/auth/auth.ts:128`, `orgs/orgs.ts:120`). The `orgs` table (`0001_tenancy.ts:59-75`)
carries no plan/status/subscription/trial/seat/quota column, there is **no entitlement table across any
migration**, and authorization is purely capability-based RBAC — never tier/entitlement-based. Every
authenticated org is fully unlimited. (Distinct from PAY/payout-sync, which is the tenant billing _their
own_ customers — that is customer-facing AR/AP, never a charge to use OpenBooks.)

Critical fork to settle first: **gating is a hosted-plane concern only — self-host must stay ungated.**
The open-source self-host build (OB-229…231) cannot be paywalled, so entitlement enforcement must be
**config-gated and off by default**, engaged only on the OpenBooks-run hosted plane. This mirrors the
provider-seam idiom (`in-process` vs a hosted adapter).

What the build needs:

- **An entitlement/subscription model** — a per-org table (plan, status `trialing`/`active`/`past_due`/
  `canceled`, current-period bounds, the platform Stripe customer + subscription ids, and the
  cap-and-upgrade **connection quota** from the owner pricing decision at `ROADMAP.md:1194-1199`).
  New table, `BINARY(16)`/composite-key tenancy per house rules; almost certainly a **mutable** table
  (status changes over the subscription lifecycle), so it needs a `MUTABLE_TABLES` line and the app-role
  UPDATE grant — unlike most of the ledger.
- **A billing step in / after signup** — Stripe Checkout or Billing **on OpenBooks' own platform Stripe
  account** (a dedicated account under our legal entity, kept separate from any customer-facing keys),
  wired so `register` either provisions a trial or requires a completed checkout before the org becomes
  usable. Reuse the `providers/payment/stripe.ts` fetch-only adapter idiom — no vendor SDK.
- **A webhook→entitlement sync** — a signature-verified inbound Stripe **billing** webhook
  (`customer.subscription.*`, `invoice.payment_failed`, …) that keeps the org's entitlement row current.
  This is a _second_ inbound webhook subsystem distinct from the payment-processing one; reuse its
  two-level idempotency + `FOR UPDATE` serialization pattern (`payments-processing/webhook.service.ts`).
- **An `assertEntitled` service-layer seam** — the entitlement analog of `requirePermission` (capability
  vs paid-tier are orthogonal): a hosted-only guard that refuses writes / feature access when a plan is
  `past_due`/`canceled` or a quota (e.g. connected institutions) is exceeded. Transport stays logic-free
  (the requirePermission precedent). Decide the failure vocabulary (a `402`/`payment_required` family)
  and whether read-only survives a lapsed subscription.
- **Signup hardening that comes with it** — email verification and/or an anti-abuse gate on open
  registration (today there is none: `users.is_active` defaults to 1, no pending/verification state),
  since a paid funnel changes the abuse surface.

Depends on the hosted plane OB-235 defines (billing runs there, not in self-host) and shares its Stripe/
secrets seams; the metering half overlaps OB-235's per-org FC-usage note (`ROADMAP.md:1170-1186`).
Deliverable is a decision `D-NN` + the build, gated on the M7 pricing decision being made. Until then the
product ships **open self-service** (the current behavior) and gating is a hosted-launch prerequisite,
not a v1 item.

### Hosted — per-org sender identity + self-service domain authentication (OB-252, future)

> **Ticket-number note (2026-08-02):** renumbered from OB-251 → **OB-252** to resolve a collision with
> [Security — at-rest key rotation (OB-251)](#security--at-rest-key-rotation-via-envelope-encryption-ob-251-future),
> added the same day in another session.

**Let each org send invoices/dunning/statements from _their own_ domain, set up self-service — no
OpenBooks staff in the loop.** Today the sender is a single instance-wide `EMAIL_FROM_ADDRESS`
(`config/config.ts:200`, `demand()`-ed, no default), and `EmailProvider.send()`
(`plugin-api/src/providers.ts:42`) carries only `{ to, subject, text, html? }` — **no `from`, no
`reply-to`** — so on the hosted plane every tenant's mail would go out as _OpenBooks'_ address, not the
customer's. That is a real trust + deliverability gap (mail from `openbooks.app` rather than
`their-company.com`, and a client can't reply to reach the actual business). `org_branding` has no
sender fields.

Recommended provider: **Postmark** (verified against current docs, 2026-08-02). Its
[Domains API](https://postmarkapp.com/developer/api/domains-api) is built for a platform sending on
behalf of many customers and makes the flow fully programmatic — verify a domain once and _any_ address
on it can send (no per-address confirmation), with no documented cap on domains per account. The
self-service flow: org admin enters their domain → OpenBooks `POST /domains` → surface the returned
**DKIM** (`DKIMTextValue`) + **Return-Path** (`ReturnPathDomainCNAMEValue`, the SPF/DMARC-alignment
CNAME) records → user adds them at their DNS host → **Verify** button calls `PUT …/verifyDkim` +
`PUT …/verifyReturnPath`. The one manual step (adding DNS records) is the _user's_ action on their _own_
domain and is inherent to domain auth with any provider (SES/Mailgun included); it also _is_ the proof
of domain control.

Two safety requirements this design must not skip:

- **Bind a verified domain to the org that proved control, and enforce it at send time.** All tenant
  domains live under one Postmark account, so Postmark itself won't stop org A sending as `@orgB.com`
  — OpenBooks must record the org↔domain binding on verification and refuse a `from` outside the
  caller org's verified set. (A cross-org send here would be spoofing; keep it in the same register as
  the 404-not-403 tenant-isolation rules.)
- **A working default sender _before_ verification, so custom-domain auth is an upgrade, not a gate.**
  Send from an OpenBooks-owned, already-authenticated domain (e.g. `notifications@mail.openbooks.app`)
  with the org's name as display name and their address as **reply-to**, so every org can send from day
  one and only _upgrades_ to their own domain when they want the trust/deliverability win.

The build: (1) extend the `EmailProvider.send()` message with `from` + `replyTo` (interface change in
`plugin-api`, re-export the type); (2) a **`postmark`** adapter — `fetch`-based with an
`X-Postmark-Server-Token` header, **no SDK** (narrowest-surface rule), and route transactional mail
through a **transactional Message Stream**, never broadcast; (3) a per-org sender model — an
`org_branding` sender field plus a **mutable** `org_sending_domains` table tracking Postmark `domainId`

- DKIM/Return-Path status (needs a `MUTABLE_TABLES` line + the app-role UPDATE grant); (4) the setup
  screen + a verify-poll (or Postmark webhook) to flip status.

Scope: this is a **hosted-plane feature** — OpenBooks holds the Postmark server token and brokers
everything. A self-host on plain SMTP (the OB-229 default) manages DKIM at its own provider and gets
_no_ per-org API flow; a self-host that configures Postmark gets the same self-service. Extends the
[SELFHOST email seam](#milestone-self-host--production-self-host-readiness-selfhost) (OB-229 — add
`postmark` alongside `smtp`/`ses`) and ties to the hosted entitlement plumbing
([OB-243](#hosted--subscription-gated-signup--tenant-billingentitlement-ob-243-future--the-m7-pricing-surface)).
Tripwire checklist when built: the new mutable table (grants/tenancy/harness literals), the new
domain-setup/verify routes (permission-matrix, route-table, cross-org, `openapi.json` + web
`schema.d.ts`), and a sender-identity permission decision (likely reuse `orgs.write`). Deliverable is a
decision `D-NN` + the build.
...
OB-098 pre-prod security review — external not budgeted, **internal static analysis done 2026-08-02**,
findings under the [OB-098 item](#wave-1--identity-and-the-event-backbone-3-parallel) — and change-feed emission
on base `postJournal`). The scope below is the as-built record.
...
**OB-098 security review — internal static analysis (2026-08-02).** The dedicated _external_ review
this ticket flagged as owed is **not budgeted** — in its place, a full static read of the surface
was performed (service, credential minting, PKCE, the bearer-identity resolver, the route layer,
cookie attributes, and the atomic DB guards). **No critically exploitable-today defect was found**;
the flow's usual failure modes are closed: opaque DB-backed SHA-256 tokens, mandatory PKCE `S256`
(`plain` unmodelled), exact `redirect_uri` match, single-use codes via an atomic
`UPDATE … WHERE consumed_at IS NULL` inside the mint transaction, refresh-token rotation, live
scope∩role recomputation with client-deactivation/revocation/membership re-checked on every bearer
use, a non-oracle RFC 7009 revoke, and a mint-nothing `authorize` GET. Five hardening findings, none
a launch blocker on its own, ranked; **a static read is not a substitute for dynamic/fuzz testing —
these are the code-level results only:**

| #   | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Severity    | Effort  |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------- |
| 1   | **Consent-endpoint CSRF rests solely on `SameSite=Lax`.** `POST /oauth/consent` is session-cookie auth'd, form-urlencoded (deliberately, to follow the 302), with **no CSRF token, no Origin/Referer check, no `Idempotency-Key`** — the app's "JSON + Idempotency-Key" CSRF story (`cookie.ts`) explicitly does not cover it. Lax blocks the cross-site POST _today_ (not exploitable now), but any later move to `SameSite=None`, a widened `cookieDomain`, or a browser mishandling Lax turns it directly into consent-CSRF → silent delegation of the victim's scoped access to an attacker-controlled client. **Fix:** add an Origin/Referer allowlist check (or per-session CSRF token) on the consent POST — defense-in-depth so the guarantee isn't one cookie attribute deep. | **Medium**  | Low     |
| 2   | **Client secrets are minted/stored but never verified.** `registerOAuthClient` returns a `clientSecret` and stores `secret_hash`, but no code path ever compares a presented secret — both token paths authenticate the client by matching the _public_ `client_id` only. A valid public-client PKCE model, but the issued secret confers nothing (misleading), and there is no confidential-client option: the **refresh grant needs no secret**, so a stolen refresh token + the non-secret `client_id` mints fresh tokens. **Fix:** drop the secret and document these as public clients, _or_ add `token_endpoint_auth_method` and verify `secret_hash` (constant-time) at the token endpoint for confidential clients.                                                            | Low–Med     | Low–Med |
| 3   | **No refresh-token reuse detection / family revocation.** Rotation makes a replayed _old_ token fail, but presenting an already-rotated refresh token only returns `invalid_grant` — it does not revoke the live descendant, so a thief who beats the legitimate client to the rotation leaves both with live tokens until noticed. OAuth 2.1 BCP wants the whole family revoked on reuse. Needs a small schema add (`refresh_token_id` is `null` on refresh rows today — no chain).                                                                                                                                                                                                                                                                                                   | Low         | Med     |
| 4   | **Missing schema input hardening** (`shared-types/platform/oauth.ts`): `redirectUris`/`redirect_uri` are `z.url()` with **no scheme restriction** (`javascript:`/`data:`/plain `http://` all pass; low-risk given admin-only registration + exact match + browsers refusing non-http(s) `Location`) — constrain to `https` + loopback. And `codeChallenge`/`code_verifier`/`scope` are unbounded `z.string()` — add RFC 7636's 43–128-char verifier bounds and a `scope` max as input hygiene.                                                                                                                                                                                                                                                                                         | Low         | Low     |
| 5   | **No rate limiting** on the OAuth token / consent / authorize endpoints. Not a confidentiality risk (256-bit secrets defeat brute force), but no abuse/DoS backstop; platform-wide (login too). Belongs at the reverse proxy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Low / infra | Infra   |

Recommended order before the AS fronts third-party clients not under our control: **#1 and #4 first**
(an afternoon, add-only), then settle **#2** as an explicit public-vs-confidential decision, with #3
and #5 as follow-ups. Nothing here blocks the current internal-only deployment.
