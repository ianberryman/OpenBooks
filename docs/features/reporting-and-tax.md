...
| **Aging** (`aging.service.ts`)                               | AR/AP outstanding as-at a date                                              | Bucket sums tie **exactly** to the control-account balance (decision **D-40**, criterion C8); includes unapplied credits as a negative "current" line; scoped to one contact it **is** the customer statement of account (see [Sales / AR](sales-ar.md#statement-of-account)). |
...
`GET /v1/reports/export?report=&format=csv|xlsx` (`modules/reports/export`) runs the **same** report
service the JSON route runs, flattens the result through a per-report adapter into a canonical
`TabularReport`, and serialises it — so an export can never disagree with the on-screen figures. Both
serialisers are **hand-rolled and dependency-free** (the repo carries no csv/xlsx/zip library and adds
none): RFC-4180 CSV, and a real `.xlsx` assembled as a store-only ZIP of minimal inline-string OOXML.
The v1 set is trial balance, P&L, balance sheet, general ledger (paged through in full), aging, cash
flow, and budget-vs-actual; gated by `reports.read` like every report. A web Export control sits in the
Reports shell. Document-list export and projection/audit export are deferred follow-ups.
