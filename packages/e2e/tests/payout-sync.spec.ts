 ...
 *
 * ## OB-237b correction: a manual payout is recorded as a visible skip, never posted
 *
 * Live against real Stripe, OB-237's `balance_transactions?payout=` breakdown 400s on a
 * **manually**-created payout — and so does the Reporting-API `payout_reconciliation`
 * report. A manual payout's amount is arbitrary and tied to no set of transactions, so it
 * has **no per-payout breakdown in any Stripe API** (ROADMAP's OB-237b correction). So a
 * manual payout is recorded as a *visible* `skipped` row carrying an actionable reason —
 * never posted, never a swallowed failure. `providers/payment/fake.ts`'s own header names
 * the deterministic trick this narrative relies on: a payout **object id containing the
 * substring `'manual'`** resolves `{ kind: 'unsupported' }`, the real adapter's behaviour
 * for `automatic: false`. Full manual-payout support (period/balance-driven recognition)
 * is OB-237c, out of scope here.
 */
 ...
 * default here too; the OB-237b manual-payout scene below passes `true`, though a
 * manual payout is recorded as a visible skip regardless of `autoPost` (OB-237b
 * correction) — the flag would only matter for an automatic payout on that connection.
 */
...
test('a Stripe payout syncs to one grossed-up summary journal, reviewed and posted, and a redelivered webhook does not double it', async ({
  ...
  // API, so it is recorded as a visible skip, never posted. See this file's header.
  // -------------------------------------------------------------------------
  ...
  await test.step('OB-237b: a manual payout is recorded as a visible skip, never posted', async () => {
    ...
    // *containing* `'manual'` resolves `{ kind: 'unsupported' }` — the real adapter's
    // behaviour for `automatic: false`. Dispatch still answers `'processed'`: the event
    // was handled; the payout sync it produced is a visible skip, not a swallowed failure.
    ...
    expect(sync.journalId).toBeNull();
    expect(sync.skipReason).toBe(
      'manual_payout_unsupported:summary_sales requires automatic payouts',
    );
    expect(sync.breakdown).toHaveLength(0);

    // Nothing posted — this connection's own clearing account is untouched, and no
    // summary journal exists for the manual payout.
