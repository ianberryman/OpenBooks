...
/**
 * The harness this screen's tests share — copied from `recurring-invoices/test-support.tsx`
 * rather than imported, the same self-containment reason `queries.ts` gives: no screen
 * folder imports another's, so each carries its own small copy of the harness that reaches
 * it.
 *
 * ## What is stubbed, and what deliberately is not
 *
 * The API is stubbed at `fetch` and nowhere higher — the generated client, `unwrap`,
 * `ApiError`, `presentApiError` and TanStack Query are all the real thing.
 *
 ...
 * replaces globals nothing will ever read again, so the replacement happens here, at the top
 * level of a module that pulls in nothing from `src/api/`, and a test file imports the
 * screen it exercises with `await import(...)` *after* this module has been evaluated.
 *
 * `Request` is replaced because `API_BASE_URL` is empty in this package's default
 * (same-origin) configuration, so the client builds `new Request('/v1/pay-bills')`, and
 * undici's `Request` — unlike a browser's — refuses a relative URL rather than resolving it
 * against the document. The shim resolves it against `location.href` and changes nothing
 * else.
 *
 * `MemoryRouter` wraps every render here because `pay-bills.tsx` links to `/disbursements`
 * with `<Link>` — a `<Router>`-less render throws the moment that renders, the same reason
 * `bill-captures`' own harness wraps one.
 */

export interface StubCall {
  readonly method: string;
  readonly path: string;
  /** The raw querystring, e.g. `?status=open` — `''` when there was none. */
...
export interface StubRoute {
  ...
  /** `/v1/pending-payments/:pendingPaymentId` — the shape the server declares its own routes with. */
  readonly path: string;
...
/**
 * Swaps in the route table for one test. Routes are matched in order, so a test can prepend
 * a refusing route to override a succeeding one.
 */
