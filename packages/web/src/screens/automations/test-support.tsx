 ...
 * rather than imported for the self-containment reason `queries.ts`' header gives: no
 * screen folder imports another's, so each carries its own small copy of the harness that
 * reaches it.
 *
 ...
 * (same-origin) configuration, so the client builds `new Request('/v1/automations')`, and
...
export interface StubCall {
  ...
  /** The raw querystring, e.g. `?limit=200` — `''` when there was none. */
...
export interface StubRoute {
  ...
  /** `/v1/automations/:automationId` — the shape the server declares its own routes with. */
  readonly path: string;
