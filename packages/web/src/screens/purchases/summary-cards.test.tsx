...
import { BillsSummaryCards } from './summary-cards';
import type { BillCardFilter } from './ap-document';
import type { BillsSummary, BillsSummaryResult } from './queries';

/**
 * The bills-list summary cards (OB-069 UI).
 *
 ...
 * loading and error states stand in without throwing, and that each card toggles its filter
 * and reflects the active one in its pressed state.
 */
const DATA: BillsSummary = {
  asOf: '2026-07-31',
  totalUnpaid: '34378',
  openCount: 3,
  totalOverdue: '112500',
...
function result(partial: Partial<BillsSummaryResult>): BillsSummaryResult {
  return { data: null, isPending: false, error: null, refetch: () => {}, ...partial };
}

function cards(
  partial: Partial<BillsSummaryResult>,
  props?: {
    activeFilter?: BillCardFilter | null;
    onSelectFilter?: (filter: BillCardFilter) => void;
  },
) {
  return (
    <BillsSummaryCards
...
describe('BillsSummaryCards', () => {
  it('renders the three figures as grouped currency', () => {
    ...
    expect(screen.getByText('Across 3 open bills')).toBeInTheDocument();
    expect(screen.getByText('2 overdue')).toBeInTheDocument();
  });

  it('singularises the open-bill count', () => {
    render(cards({ data: { ...DATA, openCount: 1 } }));

    expect(screen.getByText('Across 1 open bill')).toBeInTheDocument();
  });

  it('shows a placeholder rather than a figure while loading', () => {
    render(cards({ data: null, isPending: true }));
