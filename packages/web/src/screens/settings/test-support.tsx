...
/**
 * The harness the three settings tests share (OB-058's jsdom environment).
 *
 ...
 * which is the same objection spec §11 raises to mocking the database.
 *
 ...
 * (same-origin) configuration, so the client builds `new Request('/v1/members')`, and
 ...
 * else, so the route table matches the URL a browser would have produced.
 */
...
export interface StubRoute {
  ...
  /** `/v1/members/:userId` — the shape the server declares its own routes with. */
  readonly path: string;
...
/**
 * `details.precondition` is what makes a refusal actionable rather than merely red — it is
 * the stable token `PreconditionFailedError` validates on the server, and the two refusals
 * these tests are about (`dimension_value_in_use`, `last_owner_in_org`) are only legible
 * because the client can branch on it.
 */
export function preconditionFailed(precondition: string, message: string): StubReply {
  return { status: 412, body: apiError('precondition_failed', message, { precondition }) };
}
