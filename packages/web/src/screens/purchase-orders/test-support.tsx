 ...
 * `useFixedAssetReferenceData` gives: no screen folder imports another's, so each carries
 ...
 * (same-origin) configuration, so the client builds `new Request('/v1/fixed-assets')`, and
...
export interface StubCall {
  ...
  /** The raw querystring, e.g. `?status=active&limit=200` — `''` when there was none. */
...
export interface StubRoute {
  ...
  /** `/v1/fixed-assets/:fixedAssetId` — the shape the server declares its own routes with. */
  readonly path: string;
 ...
 */
export function renderWithQueryClient(ui: ReactElement, initialPath = '/purchase-orders'): void {
  ...
  // The screen is routed (`/purchase-orders` list, `/purchase-orders/:id` document), so it
  // mounts under a `MemoryRouter` nested as `App.tsx` nests it — a `path="/purchase-orders/*"`
  // route — and a `/purchases/*` marker stands in for the bill screen that converting a
  // purchase order navigates to.
  render(
    <QueryClientProvider client={client}>
      <MemoryRouter initialEntries={[initialPath]}>
        <Routes>
          <Route path="/purchase-orders/*" element={ui} />
          <Route path="/purchases/*" element={<div>Bill page</div>} />
        </Routes>
      </MemoryRouter>
    </QueryClientProvider>,
  );
