...
import type { OrderLineDraft } from './order-state';

type CatalogItem = components['schemas']['CatalogItem'];

/**
 * A catalog item's defaults applied to a purchase-order line (D-CAT-2). Each default falls back
 * to what the line already holds; `catalogItemId` records the provenance and nothing rereads
 * it. The order line holds its price as `unitAmountMinor`.
 */
export function applyCatalogItem(line: OrderLineDraft, item: CatalogItem): OrderLineDraft {
  return {
    ...line,
...
/**
 * One purchase-order line — the AP-side mirror of `estimates/line-row.tsx`, itself
 * `recurring-invoices/line-row.tsx`'s shape minus the tax-rate column: this screen offers no
 * tax-rate picker on a line at all (see `order-state.ts`'s file header for why `taxRateId`
 * still travels, carried and fixed, regardless), and — per `PredocumentLineRequestInput`'s own
 * description (D-M7) — there is no dimension column either.
 */
export interface LineRowProps {
  readonly line: OrderLineDraft;
  ...
  readonly onChange: (line: OrderLineDraft) => void;
      ...
      <td className="w-24 p-1">
        <Field error={fieldErrors[`${path}.quantity`]}>
          <TextInput
            ...
            // `type="text"` with `inputMode="decimal"`: a number input silently discards what
            // it cannot parse, and a quantity is multiplied by a price before anyone sees the
            // result (`MoneyInput`'s reason, applied to a multiplier).
            inputMode="decimal"
            autoComplete="off"
            value={line.quantity}
            disabled={disabled}
...
/**
 * The compact (`< md`) presentation of one line: a stacked card, so the four-column entry row
 * `LineRow` renders becomes full-width fields instead (mirrors `sales/line-row.tsx`'s
 * `LineCard`, minus the tax-rate field and the computed net/tax/gross line that a purchase-order
 * line has neither of — see this file's header). It takes the identical `LineRowProps` `LineRow`
 * does, so the two presentations cannot drift on a handler or an `aria-label`.
 */
