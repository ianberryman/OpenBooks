 ...
 * rather than imported for the self-containment reason `queries.ts`'s
 * `useTemplateReferenceData` gives: no screen folder imports another's, so each carries its
 * own small copy of the harness that reaches it.
 *
 ...
 * (same-origin) configuration, so the client builds `new Request('/v1/recurring-journals')`,
...
export interface StubRoute {
  ...
  /** `/v1/recurring-journals/:templateId` — the shape the server declares its own routes with. */
  readonly path: string;
