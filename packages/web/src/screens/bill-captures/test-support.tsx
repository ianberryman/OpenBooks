...
/**
 * The harness OB-189's tests use, copied from `settings/test-support.tsx` rather than
 * imported from it: screens in this package are self-contained (`sales/queries.ts`'s own
 * reason — a shared module makes one screen's change another screen's problem), and this
 * ticket's scope is `screens/bill-captures*` only. The content below is generic — nothing
 * in it is settings-specific — so the duplication is the harness itself, not a second
 * design. `dunning/test-support.tsx` and `recurring-invoices/test-support.tsx` are the
 ...
 * production — `unwrap`, `ApiError`, `presentApiError`, and TanStack Query.
 *
 ...
 * Getting that order wrong does not fail quietly: every request 500s with "Failed to parse
 * URL", which is the next paragraph.
 *
 * `Request` is replaced because `API_BASE_URL` is empty in this package's default
 * (same-origin) configuration, so the client builds `new Request('/v1/bills/captures')`,
 ...
 * changes nothing else, so the route table matches the URL a browser would have produced.
 *
 * ## `MemoryRouter`
 *
 * `BillCapturesScreen` renders a `react-router-dom` `<Link>` (to `/purchases`, on a
 * drafted capture's row and on the post-review success notice), which throws outside a
 * router context. `MemoryRouter` supplies one without touching browser history, matching
 * `banking/banking.test.tsx`'s reason for the same wrapper.
 */
...
export interface StubRoute {
  ...
  /** `/v1/bills/captures/:captureId` — the shape the server declares its own routes with. */
  readonly path: string;
