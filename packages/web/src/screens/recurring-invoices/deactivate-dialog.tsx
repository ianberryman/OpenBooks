...
import type { RecurringInvoiceTemplate } from './queries';
import { useDeactivateTemplate, useIntentKey } from './queries';

/**
 * Retiring a template — the one-way door, separate from pause/resume (`queries.ts`'
...
function DeactivateTemplateContent({
  ...
  readonly template: RecurringInvoiceTemplate;
  ...
  return (
    <DialogContent
      title="Retire this recurring invoice?"
      description="The engine stops raising invoices from it. This cannot be undone from here."
      footer={
        <>
          <DialogClose asChild>
    ...
    >
      <div className="flex flex-col gap-3 text-base text-text-muted">
        <p>
          ...
          materialise new invoices. Nothing already raised from it is affected — every cycle it has
