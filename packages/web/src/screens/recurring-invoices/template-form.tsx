...
/**
 * The one form for creating and editing a recurring invoice template (OB-133; OB-128,
 * D-75, D-76).
 *
 * ## `startDate` is a create-only field because it is a create-only value
 *
 * It never appears on `RecurringInvoiceTemplate` at all — it seeds `nextRunDate` once and
 ...
 *
 * ## Nothing here prices a line
 *
 * A template line carries `accountId`, `quantity`, `unitAmount` and an optional
 * `taxRateId` — exactly what `createInvoice` needs to price a cycle — and nothing else.
 * There is no net/tax/gross to compute or show, because a template is not a posted
 * document (`RecurringInvoiceLine`'s own words); the arithmetic happens once per cycle, on
 * the server, when a cycle actually materialises.
 *
 * ## `isActive` is not a field on this form
 *
 * Pause, resume and retire are list-level controls (`queries.ts`'
 ...
 * would make "I fixed the price" and "I paused this customer's billing" the same click.
 */
...
export interface TemplateFormDialogProps {
  ...
  readonly template: RecurringInvoiceTemplate | null;
...
function TemplateFormContent({
  ...
  const contactOptions = useMemo<ComboboxOption[]>(
    () =>
      reference.contacts.map((contact) => ({
        ...
        // Archived contacts are listed and disabled rather than omitted, so a template
        // that already names one still shows which customer it bills.
  ...
  return (
    <DialogContent
      title={template === null ? 'New recurring invoice' : 'Edit recurring invoice'}
      description="A customer, a schedule, and how to raise the invoice each cycle."
      className="max-w-3xl"
      footer={
        <>
          <DialogClose asChild>
      ...
      >
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <Field
            ...
            hint="What an operator picks this template out by, not a line."
          >
        ...
        <Field hint={MATERIALIZATION_MODE_EXPLANATIONS[state.materializationMode]}>
          ...
          <Select
            ...
            disabled={pending}
            onValueChange={(value) => {
              if (value === 'draft' || value === 'approved') edit({ materializationMode: value });
            }}
          />
        </Field>
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <Field
            ...
            hint="dueDate = issueDate + this, each cycle."
          >
            ...
            <TextInput
              ...
              disabled={pending}
              onChange={(event) => {
                edit({ dueDays: event.target.value });
              }}
            />
          </Field>
        </div>
        ...
        <Field hint={TAX_MODE_EXPLANATIONS[state.taxMode]}>
          ...
          <Select
            ...
            disabled={pending}
            onValueChange={(value) => {
              if (value === 'exclusive' || value === 'inclusive') edit({ taxMode: value });
            }}
          />
        </Field>

        <Field error={fieldErrors['memo']} hint="Optional.">
