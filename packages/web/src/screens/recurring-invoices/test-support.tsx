...
/**
 * The harness this screen's tests share — `settings/test-support.tsx`, copied rather than
 * imported for the self-containment reason `queries.ts`' `useTemplateReferenceData` gives:
 * no screen folder imports another's, so each carries its own small copy of the harness
 * that reaches it.
 *
 ...
 * (same-origin) configuration, so the client builds `new Request('/v1/recurring-invoices')`,
...
export interface StubCall {
  ...
  /** The raw querystring, e.g. `?type=revenue&limit=200` — `''` when there was none. */
...
export interface StubRoute {
  ...
  /** `/v1/recurring-invoices/:templateId` — the shape the server declares its own routes with. */
  readonly path: string;
