...
/**
 * The harness this screen's tests share — `bill-captures/test-support.tsx`, copied rather
 * than imported for the self-containment reason `queries.ts`' header gives: no screen
 ...
 * (same-origin) configuration, so the client builds `new Request('/v1/work-items')`, and
 ...
 * ## `MemoryRouter`
 *
 * `WorkQueueScreen` renders a `react-router-dom` `<Link>` (to `/agent-proposals`, on any row
 * whose `proposedDraftId` is set), which throws outside a router context. `MemoryRouter`
 * supplies one without touching browser history, matching `bill-captures/test-support.tsx`'s
...
export interface StubRoute {
  ...
  /** `/v1/work-items/:workItemId` — the shape the server declares its own routes with. */
  readonly path: string;
