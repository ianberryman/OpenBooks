...
/**
 * The harness this screen's tests share — `budgets/test-support.tsx`, copied rather than
 * imported for the self-containment reason `queries.ts`'s own header gives: no screen
 ...
 * (same-origin) configuration, so the client builds `new Request('/v1/statement-packages')`,
 * and undici's `Request` — unlike a browser's — refuses a relative URL rather than resolving
 * it against the document. The shim resolves it against `location.href` and changes nothing
 * else.
 */
...
export interface StubRoute {
  ...
  /** `/v1/statement-packages` — the shape the server declares its own routes with. */
  readonly path: string;
