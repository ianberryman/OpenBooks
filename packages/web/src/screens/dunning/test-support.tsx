...
/**
 * The harness OB-132's tests use, copied from `settings/test-support.tsx` rather than
 ...
 * ticket's scope is `screens/dunning*` only. The content below is generic — nothing in it
 * is settings-specific — so the duplication is the harness itself, not a second design.
 *
 ...
 * (same-origin) configuration, so the client builds `new Request('/v1/dunning-policies')`,
...
export interface StubRoute {
  ...
  /** `/v1/dunning-policies/:policyId` — the shape the server declares its own routes with. */
  readonly path: string;
