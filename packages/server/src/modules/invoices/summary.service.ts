import type { InvoicesSummary, InvoicesSummaryQuery } from '@openbooks/shared-types';
import { invoicesSummaryQuerySchema } from '@openbooks/shared-types';

import { getContext } from '../../context';
import type { RequestContext } from '../../context';
import { InternalError, parseInput } from '../../errors';
import { requirePermission } from '../permissions';
import { selectArDocuments, selectPayments } from '../reports/aging.repository';

import { orgScope } from './ar-documents.repository';

/**
 * The three headline figures the invoices list shows (OB-069 UI): what is still
 * owed, what of it is overdue, and what has been received in the last 30 days. The
 * AR mirror of `bills/summary.service.ts`; see there for the arguments that hold
 * here too.
 *
 ...
 * would drift from the aging report the moment either changed. So the invoice basis
 * is `selectArDocuments` and the received figure is `selectPayments`, the same
 * reads the receivable aging is assembled from, and `totalUnpaid` equals that
 * report's invoice total by construction rather than by luck.
 *
 * The web list cannot sum it either: it is one capped page (D-list-limit), so a
 * browser total would be wrong past the first hundred invoices. This is the server
 * doing the arithmetic once, over every invoice, which is the only place it is
 * right.
 *
...
export async function invoicesSummary(
  query: InvoicesSummaryQuery = {},
  ctx: RequestContext = getContext('invoicesSummary()'),
): Promise<InvoicesSummary> {
  await requirePermission(ctx, 'invoices.read');
  const request = parseInput(invoicesSummaryQuerySchema, query);
  ...
  for (const invoice of invoices) {
    const outstanding = invoice.total - invoice.allocated;
    // A settled invoice contributes zero and is not an open item — the same line
    ...
    if (invoice.dueDate !== null && invoice.dueDate < asOf) {
  ...
  for (const payment of payments) {
    // The full payment amount — money in from a customer — whatever it was applied to.
