...
// between the invoice renderer, the statement-package renderer, and this one.
// Duplicating it would be a second copy to keep in sync with whatever pdfmake
// version the project runs; importing it costs one edge in the dependency graph
// instead. `.dependency-cruiser.cjs` has no rule against a module reaching a
// sibling module's non-barrel file directly, and nothing about `fonts.ts` is
// delivery-specific (see its own header) — `statements/renderer/index.ts` already
// takes this same import for the same reason.
import { loadDefaultFonts } from '../../delivery/renderer/fonts';

import { buildStatementDocDefinition } from './document';
import type { StatementRenderInput, StatementRenderer } from './types';

export type { StatementRenderInput, StatementRenderer };
export { buildStatementDocDefinition };

/**
 * The pdfmake-backed customer-statement renderer (OB-220; ROADMAP D-71 locks
 * pdfmake, "not headless Chromium" — the same choice `delivery/renderer/index.ts`
 * and `statements/renderer/index.ts` were built against, restated here rather than
 * shared because the three renderers build unrelated document definitions and have
 * no other reason to depend on each other).
 *
 * `createStatementRenderer()` is the only export application code should call;
 * everything else in this directory is this file's implementation, mirroring
 * `statements/renderer/index.ts` line for line where the reasoning is identical —
 ...
 * alone — would make two renders of the identical `StatementRenderInput` differ
 * byte-for-byte purely on wall-clock time. `DETERMINISTIC_CREATION_DATE` overrides
 * it directly on the pdfkit document pdfmake hands back, for
 * `statements/renderer/index.ts`'s exact reason: `createPdfKitDocument` forwards
 ...
 * `document.ts` itself reads the clock only through `StatementRenderInput`'s
 ...
 * and `statements/renderer/index.ts` already assume, applied to a third document —
 * nothing here is a new assumption about the library, only a third call site for
 * the same one:
 *
 ...
 *   and statement-package renderers, so this assumption is verified the moment any
 *   of the three typechecks.
 */

/**
 * The slice of pdfkit's `PDFDocument` this file touches. Named locally rather than
 * sourced from `@types/pdfkit` — see the header above for why. Identical to
...
/**
 * A fixed instant, not `new Date(0)` — `delivery/renderer/index.ts`'s reasoning: an
 * epoch timestamp on every generated PDF reads as a bug report waiting to happen
 * the first time someone opens its document properties.
 */
...
/** The pdfmake-backed `StatementRenderer` (OB-220). See this file's header. */
export function createStatementRenderer(): StatementRenderer {
  ...
  return {
    render(input: StatementRenderInput): Promise<Uint8Array> {
      const docDefinition = buildStatementDocDefinition(input);
